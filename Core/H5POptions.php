...
use OpenLMS\H5PBundle\Entity\Option;
...
class H5POptions
{
    ...
    public function setOption($name, $value)
    {
        ...
        if (!isset($this->storedConfig[$name]) || $this->storedConfig[$name] !== $value) {
            ...
            $option = $this->manager->getRepository('H5PBundle:Option')->find($name);
            if (!$option) {
    ...
    private function retrieveStoredConfig()
    {
        if ($this->storedConfig === null) {
            $this->storedConfig = [];
            $options = $this->manager->getRepository('H5PBundle:Option')->findAll();
