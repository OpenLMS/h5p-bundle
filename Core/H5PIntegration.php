...
class H5PIntegration
{
    ...
    public function getGenericH5PIntegrationSettings(): array
    {
        ...
        // Create AJAX URLs
        $setFinishedUrl = $this->router->generate('openlms_h5p_interaction_setfinished', ['token' => \H5PCore::createToken('result')]);
        $contentUserDataUrl = $this->router->generate('openlms_h5p_interaction_contentuserdata', ['contentId' => ':contentId', 'dataType' => ':dataType', 'subContentId' => ':subContentId', 'token' => \H5PCore::createToken('contentuserdata')]);
    ...
    public function getH5PContentIntegrationSettings(Content $content): array
    {
        ...
        if (is_object($this->tokenStorage->getToken()->getUser())) {
            $contentUserData = $this->entityManager->getRepository('H5PBundle:ContentUserData')->findOneBy(['mainContent' => $content, 'user' => $this->tokenStorage->getToken()->getUser()->getId()]);
            if ($contentUserData) {
        ...
        $embedUrl = $this->router->generate('openlms_h5p_interaction_embed', ['content' => $content->getId()], UrlGeneratorInterface::ABSOLUTE_URL);
