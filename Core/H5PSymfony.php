...
namespace OpenLMS\H5PBundle\Core;

use Doctrine\ORM\EntityManager;
use Doctrine\ORM\EntityManagerInterface;
use OpenLMS\H5PBundle\DependencyInjection\Configuration;
use OpenLMS\H5PBundle\Editor\EditorStorage;
use OpenLMS\H5PBundle\Entity\Content;
use OpenLMS\H5PBundle\Entity\ContentLibraries;
use OpenLMS\H5PBundle\Entity\LibrariesHubCache;
use OpenLMS\H5PBundle\Entity\LibrariesLanguages;
use OpenLMS\H5PBundle\Entity\Library;
use OpenLMS\H5PBundle\Entity\LibraryLibraries;
use OpenLMS\H5PBundle\Event\H5PEvents;
use OpenLMS\H5PBundle\Event\LibrarySemanticsEvent;
...
class H5PSymfony implements \H5PFrameworkInterface
{
    ...
    {
        $libraries = $this->manager->getRepository('H5PBundle:Library')->findBy(['machineName' => $machineName]);
    ...
    {
        $res = $this->manager->getRepository('H5PBundle:Library')->findBy([], ['title' => 'ASC', 'majorVersion' => 'ASC', 'minorVersion' => 'ASC']);
    ...
    {
        $library = $this->manager->getRepository('H5PBundle:Library')->findOneBy(['machineName' => $machineName, 'majorVersion' => $majorVersion, 'minorVersion' => $minorVersion]);
    ...
    public function isPatchedLibrary($library)
    {
        ...
        return $this->manager->getRepository('H5PBundle:Library')->isPatched($library);
    }
    ...
    public function getLibraryUsage($libraryId, $skipContent = FALSE)
    {
        ...
        if ($skipContent) {
            ...
            $usage['content'] = $this->manager->getRepository('H5PBundle:Library')->countContentLibrary($libraryId);
        }

        $usage['libraries'] = $this->manager->getRepository('H5PBundle:LibraryLibraries')->countLibraries($libraryId);
    ...
    public function getLibraryContentCount()
    {
        ...
        $results = $this->manager->getRepository('H5PBundle:Content')->libraryContentCount();
    ...
    public function getLibraryStats($type)
    {
        ...
        $results = $this->manager->getRepository('H5PBundle:Counters')->findBy(['type' => $type]);
    ...
    public function getNumAuthors()
    {
        ...
        $contents = $this->manager->getRepository('H5PBundle:Content')->countContent();
    ...
    public function saveLibraryData(&$libraryData, $new = TRUE)
    {
        ...
        if ($new) {
            ...
            $library = $this->manager->getRepository('H5PBundle:Library')->find($libraryData['libraryId']);
        ...
        $languages = $this->manager->getRepository('H5PBundle:LibrariesLanguages')->findBy(['library' => $library]);
    ...
    {
        $libraries = $this->manager->getRepository('H5PBundle:LibraryLibraries')->findBy(['library' => $libraryId]);
        foreach ($libraries as $library) {
    ...
    {
        $library = $this->manager->getRepository('H5PBundle:Library')->find($libraryId);
        $this->manager->remove($library);
        $this->manager->flush();
    ...
    public function saveLibraryDependencies($libraryId, $dependencies, $dependencyType)
    {
        foreach ($dependencies as $dependency) {
            $library = $this->manager->getRepository('H5PBundle:Library')->find($libraryId);
            $requiredLibrary = $this->manager->getRepository('H5PBundle:Library')->findOneBy(['machineName' => $dependency['machineName'], 'majorVersion' => $dependency['majorVersion'], 'minorVersion' => $dependency['minorVersion']]);
    ...
    {
        $content = $this->manager->getRepository('H5PBundle:Content')->find($contentData['id']);
    ...
    {
        $library = $this->manager->getRepository('H5PBundle:Library')->find($contentData['library']['libraryId']);
    ...
    {
        $contentUserDatas = $this->manager->getRepository('H5PBundle:ContentUserData')->findBy(['mainContent' => $contentId, 'deleteOnContentChange' => true]);
    ...
    {
        $contentLibrariesFrom = $this->manager->getRepository('H5PBundle:ContentLibraries')->findBy(['content' => $copyFromId]);
        $contentTo = $this->manager->getRepository('H5PBundle:Content')->find($contentId);
    ...
    {
        $content = $this->manager->getRepository('H5PBundle:Content')->find($contentId);
        if ($content) {
    ...
    {
        $contentLibraries = $this->manager->getRepository('H5PBundle:ContentLibraries')->findBy(['content' => $contentId]);
    ...
    public function saveLibraryUsage($contentId, $librariesInUse)
    {
        ...
        foreach ($librariesInUse as $dependency) {
            ...
            $library = $this->manager->getRepository('H5PBundle:Library')->find($dependency['library']['libraryId']);
    ...
    {
        $library = $this->manager->getRepository('H5PBundle:Library')->findOneArrayBy(['machineName' => $machineName, 'majorVersion' => $majorVersion, 'minorVersion' => $minorVersion]);
        if (!$library) {
            return false;
        }
        $library['libraryId'] = $library['id'];

        $libraryLibraries = $this->manager->getRepository('H5PBundle:LibraryLibraries')->findBy(['library' => $library['id']]);
    ...
    public function loadContentDependencies($id, $type = NULL)
    {
        ...
        $contentLibraries = $this->manager->getRepository('H5PBundle:ContentLibraries')->findBy($query, ['weight' => 'ASC']);
        $dependencies = [];
        foreach ($contentLibraries as $contentLibrary) {
            /** @var Library $library */
    ...
    public function updateContentFields($id, $fields)
    {
        ...
        $content = $this->manager->getRepository('H5PBundle:Content')->find($id);
    ...
    public function clearFilteredParameters($library_id)
    {
        ...
        $contents = $this->manager->getRepository('H5PBundle:Content')->findBy(['library' => $library_id]);
...
//    \Drupal\Core\Cache\Cache::invalidateTags(['h5p_content']);
    ...
    {
        return $this->manager->getRepository('H5PBundle:Content')->countNotFiltered();
    }
    ...
    {
        return $this->manager->getRepository('H5PBundle:Content')->countLibraryContent($library_id);
    }
