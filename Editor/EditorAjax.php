...
class EditorAjax implements \H5PEditorAjaxInterface
{
    ...
    {
        return $this->manager->getRepository('H5PBundle:Library')->findLatestLibraryVersions();
    }
    ...
    public function getContentTypeCache($machineName = NULL)
    {
        ...
        if ($machineName !== NULL) {
            $contentTypeCache = $this->manager->getRepository('H5PBundle:LibrariesHubCache')->findOneBy(['machineName' => $machineName]);
            return [$contentTypeCache];
        }

        // Get all cached content types
        return $this->manager->getRepository('H5PBundle:LibrariesHubCache')->findAll();
    }
    ...
    public function getAuthorsRecentlyUsedLibraries()
    {
        ...
        if (is_object($user)) {
            $events = $this->manager->getRepository('H5PBundle:Event')->findRecentlyUsedLibraries($user->getId());
